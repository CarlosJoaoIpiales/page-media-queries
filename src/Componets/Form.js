...
const Form = ({inmueble, setInmueble}) => {
    ...
    let{idinmubles, ubicacion, sector, extension, costo, encargado, fechaDisponibilidad, tipo} = inmueble

    const handleSubmit = () => {
        extension = parseInt(extension, 100)
        costo = parseInt(costo, 100)
        idinmubles = parseInt(idinmubles, 100)
        //validación de los datos
        if (idinmubles <=0 ||ubicacion === '' || sector === '' || extension <= 0 || costo <= 0 || 
    ...
    return (
        <form onSubmit={handleSubmit}>
            <div className="mb-3">
                <label htmlFor="idinmubles" className="form-label" >Id del Inmueble</label>
                <input value={idinmubles} name="idinmubles" onChange={handleChange} type="number" id="idinmubles" className="form-control"></input>
            </div>
